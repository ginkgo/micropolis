...
GL::Tex::UnitManager GL::Tex::_unit_manager;
...
GL::Texture::Texture(const Image& image,
...
GL::Texture::Texture(int dimensions, int w, int h, int d,
...
void GL::Texture::read_back(Image& image) {
...
GLenum GL::Tex::UnitManager::get_unit()
{
    if (_unit_list == NULL)
        initialize();

    if (_available_units < 1) {
        cerr << "GL::Texture::UnitManager: "
...
void GL::Tex::UnitManager::return_unit(GLenum unit)
{
    if (_unit_list == NULL)
        initialize();
...
void GL::Tex::UnitManager::initialize()
{
    if (_unit_list != NULL)
        return;
...
GL::Tex::UnitManager::~UnitManager() {
    if (_unit_list != NULL)
        delete[] _unit_list;
}

void GL::Texture::generate_mipmaps()
{
    assert(_bound_unit != 0);
...
void GL::TextureBuffer::load(void* data)
{
    ...
    _buffer.send_subdata(data, 0, _buffer.get_size());
    _buffer.unbind();
}
